...
namespace iZINE.Web.MVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private bool IsAdminloggedIn(string username)
        {
            using (iZINEEntities DataContext = iZINE.Web.Utils.DataContextFactory.GetDataContext<iZINEEntities>())
            {
                iZINE.Businesslayer.User user = DataContext.Users.Include("Role").FirstOrDefault<iZINE.Businesslayer.User>(u => u.Membership.Username.CompareTo(username) == 0 && u.Active == true);
                if(user.Role.RoleId == new Guid("0d629cba-f55c-461a-831e-58053db7189f")) 
                    return true;
