	   ...
	   {%>
				<table>
					<tr>
						...
						<td>
							<select name="plank" id="plank">
								<option label="<%=Resources.Resource.selectEdition %>"></option>
							</select>
						</td>
						<td>
						...
						<td>
							<div style="width: 100px; display: none" id="displayPub">
								<label style="font-style: italic">
									To see publication overview</label>
								<a href="#" id="pubOverview">click here</a></div>
						</td>
					</tr>
				</table>
	...
	<script language="javascript" type="text/javascript">
             ...
             (function($) {
                 $(document).ready(function() {
                     ...
                     showPubLink = function() {
                         var sel = $('#plank option:selected').val();

                         var sel2 = $('#titel option:selected').val();
                         if (sel != undefined && sel2 != undefined && sel != '' && sel2 != '') {
                             $("#displayPub").show("fast");
                         }
                         else {
                             $("#displayPub").hide("fast");
                         }
                     }
        ...
        (function($) {
        	$(document).ready(function() {
        		...
        		Update = function() {
        			...
        			var callback = function(data, textStatus) {
        				...
        				$.each(data.items, function(i, val) {
        					...
        					$("<img>").attr({ style: "border-style: none", alt: "test", src: val.ImageUrl }).appendTo(vtd);
        					$(vtd).appendTo(vtr);
        					...
        					$.each(val.AssignmentsList, function(j, asset) {
        						...
        						var vtr1 = $('<tr style="color:#893b96;font-size:11px;font-weight:bold;font:arial,helvetica;">');

        						var vtd2 = $('<td>');

        						$("<img>").attr({ style: "border-style: none", alt: "test", src: asset.url }).appendTo(vtd2);
        						$(vtd2).appendTo(vtr1);
        						$('<td>').appendTo(vtr1);
        		...
        		$("#pubOverview").click(function(event) {
        			event.preventDefault();

        			var sel = $('#plank option:selected').val();

        			var sel2 = $('#titel option:selected').val();
        			window.location.href = "/plank/PubIndex?title=" + sel2 + "&&plank=" + sel;
        		});
