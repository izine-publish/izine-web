<%@ Page Title="" Language="C#" MasterPageFile="~/Areas/KeyUser/Views/Shared/KeyUser.Master" Inherits="System.Web.Mvc.ViewPage<MvcPaging.PagedList<iZINE.Web.MVC.Areas.KeyUser.Models.UserModel>>" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  ...
  <%if (Model != null && Model.Count > 0) 
        ...
        <table width="810px" cellpadding="2" cellspacing="0" class="bATR_Hdr_P" style="margin-top:10px">
            ...
            <tbody>
            <% foreach (iZINE.Web.MVC.Areas.KeyUser.Models.UserModel soi in Model)
               { %>
            <tr>
                <td>
                ...
                <td>
                |<%=Html.ActionLink(Resources.Resource.edit, "edituserview", new { userid = soi.UserId })%> <label>|</label> <a id="deleteItem" href="javascript: void(0);"> <%=Resources.Resource.delete%></a>
...
<% using (Html.BeginForm(Html.BeginForm("create", "KeyUsers", FormMethod.Post, new { id = "EditForm" }))) { } %>
        ...
        $("#popup").dialog(
                {
                    ...
                    overlay: {
                        ...
                        '<%=Resources.Resource.delete.ToUpper()%>': function() {
                            $(this).dialog('close');
                            var id = $(this).data("id");
                            var url = "/Users/DeleteLinkHandler/" + id;
        ...
        $(document).ready(function() {
            ...
            '</thead>');
                        ...
                        $.each(data.items, function(i, val) {
                            ...
                            $("<a>").text("<%=Resources.Resource.edit%>").attr({ href: '/KeyUser/KeyUsers/edituserview?userid=' + val.UserId }).appendTo(vtd);
                            $(vtd).append(" | ");
                    ...
                    $.post("/KeyUser/KeyUsers/getjsonlist", { Organisation: sel, page: 1 }, callback, "json");
                }
                else {
                    $("#data").html('');
                }
            ...
            '</thead>');
                    ...
                    $.each(data.items, function(i, val) {
                        ...
                        $("<a>").text("<%=Resources.Resource.edit %>").attr({ href: '/KeyUser/KeyUsers/edituserview/edituserview?userid=' + val.UserId }).appendTo(vtd);
                        $(vtd).append(" | ");
                        $("<a>").text(" <%=Resources.Resource.delete%>").attr({ userid: val.UserId, title: "", href: "javascript: void(0);" }).click(function(event) {
                            event.preventDefault(); var target = event.target; var userid = $(target).attr("userid"); var parentrow = $(this).parent().parent();
                            $("#popup").data("id", userid);
                ...
                $.post("/KeyUser/KeyUsers/getjsonlist", { Organisation: sel, page: pageclickednumber }, callback, "json");
